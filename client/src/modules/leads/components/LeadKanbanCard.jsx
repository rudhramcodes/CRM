  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-7 h-7 bg-zinc-100 rounded-full flex items-center justify-center shrink-0">
            <span className="text-primary-900 font-medium text-[11px]">{initials}</span>
          </div>
          <div className="min-w-0">
            <p className="text-sm font-medium text-primary-900 truncate">{lead.name}</p>
            {lead.company && <p className="text-[11px] text-zinc-400 truncate">{lead.company}</p>}
          </div>
        </div>
      </div>
      <div className="flex items-center justify-between gap-2 pt-1">
        ...
        {lead.email && <span className="text-[11px] text-zinc-400 truncate">{lead.email}</span>}
      </div>
      <div className="flex items-center gap-2 pt-0.5">
        {lead.assignedTo?.name && <span className="text-[11px] text-zinc-400 truncate">{lead.assignedTo.name}</span>}
      </div>
    </div>
  );
}
